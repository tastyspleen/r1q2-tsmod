...
void CL_WriteConfiguration (void)
{
	...
	FILE	*r1q2_f;
	char	path[MAX_QPATH];
	char	r1q2_path[MAX_QPATH];

	if (cls.state == ca_uninitialized)
		return;

	Com_sprintf (path, sizeof(path),"%s/config.cfg",FS_Gamedir());
	Com_sprintf (r1q2_path, sizeof(path),"%s/r1q2config.cfg",FS_Gamedir());
	f = fopen (path, "w");
	r1q2_f = fopen (r1q2_path, "w");
	if (!f)
	{
	...
	{
		Com_Printf ("Couldn't write r1q2config.cfg.\n", LOG_CLIENT);
		return;
	}

	fprintf (f, "// generated by quake, do not modify\n");
	fprintf (r1q2_f, "// generated by r1q2, do not modify\n");
	Key_WriteBindings (f);
	fclose (f);
	fclose (r1q2_f);

	Cvar_WriteVariables (path, r1q2_path);
}
